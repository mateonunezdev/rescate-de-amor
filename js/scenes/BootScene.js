export default class BootScene extends Phaser.Scene {
  ...
  create() {
    ...
    for(const theme of ['forest','garden','fortress','tower','palace']){const texture=this.textures.get(`architecture-${theme}`);['ground','platformA','platformB','bridge','landmark','gate'].forEach((name,index)=>{if(!texture.has(name))texture.add(name,0,(index%3)*512,Math.floor(index/3)*512,512,512);});}
