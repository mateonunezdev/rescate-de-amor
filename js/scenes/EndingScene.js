...
export default class EndingScene extends Phaser.Scene {
  ...
  create(){
    ...
    const collected=gameState.memories||[],cards=['card1','card2','card3'];gameState.finalLetterUnlocked=cards.every(c=>collected.includes(c));gameState.secretUnlocked=collected.filter(x=>/^fragment\d+$/.test(x)).length===12;gameState.achievements=[...new Set([...(gameState.achievements||[]),'Nuestra historia','Amor verdadero'])];
  }
  ...
  makeRescue(){
    this.rescuePlayable=true;this.villain=this.add.image(1120,555,'pecho-final',7).setScale(.88).setDepth(7);this.mateoCageRig=createMateoCageRig(this,940,510,{frame:1,mateoScale:.88,cageScale:.68,depth:12,locked:true});this.mateo=this.mateoCageRig.mateo;this.bars=this.mateoCageRig.cage;
    const ground=this.add.rectangle(640,625,1280,90,0x211523,.8).setDepth(6);this.physics.add.existing(ground,true);this.player=new Player(this,180,535);this.player.health=5;this.player.maxHealth=5;this.physics.add.collider(this.player,ground);this.paola=this.player;
    this.dialog=this.add.text(640,105,'ROMPE LAS TRES CADENAS MÁGICAS',{fontFamily:'monospace',fontSize:'25px',color:'#fff0d2',align:'center',stroke:'#32172f',strokeThickness:5}).setOrigin(.5).setDepth(40);
    this.chains=[['RECUERDOS',510],['CONFIANZA',665],['AMOR',820]].map(([name,x],i)=>{const link=this.add.image(x,540,'environment-atlas',16).setDisplaySize(78,150).setTint([0x8fcfff,0xd29bff,0xff6eaa][i]).setDepth(14);const label=this.add.text(x,445,name,{fontFamily:'monospace',fontSize:'14px',color:'#ffe9c9',backgroundColor:'#32172f',padding:{x:7,y:4}}).setOrigin(.5).setDepth(15);this.physics.add.existing(link,true);return{name,x,link,label,broken:false};});
    this.chainPrompt=this.add.text(640,650,'E · ROMPER CADENA',{fontFamily:'monospace',fontSize:'18px',color:'#fff',backgroundColor:'#702553',padding:{x:15,y:8}}).setOrigin(.5).setDepth(50).setVisible(false);this.input.keyboard.on('keydown-E',()=>this.breakNearbyChain());this.uiManager.showDialogueBubble('MATEO','¡Paola! Las cadenas reaccionan a ti.',{x:900,y:400,tail:'right',duration:2400,width:520});
  }
  breakNearbyChain(){if(!this.rescuePlayable)return;const chain=this.chains.find(c=>!c.broken&&Math.abs(this.player.x-c.x)<105);if(!chain)return;chain.broken=true;this.cameras.main.shake(110,.006);this.tweens.add({targets:[chain.link,chain.label],alpha:0,y:'+=55',angle:25,duration:420,onComplete:()=>{chain.link.destroy();chain.label.destroy();}});this.uiManager.showDialogueBubble('PAOLA',`${chain.name}. Esto es lo que nos trajo hasta aquí.`,{x:500,y:420,duration:1700,width:570});if(this.chains.every(c=>c.broken))this.openRescueCage();}
  openRescueCage(){this.rescuePlayable=false;this.chainPrompt.setVisible(false);this.mateoCageRig.setLocked(false);this.tweens.add({targets:this.bars,x:105,alpha:.25,duration:850});this.dialog.setText('LA JAULA ESTÁ ABIERTA');this.time.delayedCall(900,()=>{const world=this.mateoCageRig.getWorldTransformMatrix();this.mateoCageRig.remove(this.mateo);this.add.existing(this.mateo);this.mateo.setPosition(world.tx,world.ty+24).setFrame(2).setDepth(18);this.tweens.add({targets:this.mateo,x:690,duration:900});this.tweens.add({targets:this.player,x:610,duration:900,onComplete:()=>{this.player.setFrame(8);this.mateo.setFrame(3).setFlipX(true);this.dialog.setText('MATEO: “Sabía que vendrías.”\nPAOLA: “Siempre.”');for(let i=0;i<24;i++){const h=this.add.text(650+(Math.random()-.5)*160,520+Math.random()*70,i%3?'♥':'🌹',{fontSize:'20px',color:'#ff72ad'}).setDepth(25);this.tweens.add({targets:h,y:h.y-190,alpha:0,duration:1400+Math.random()*500,onComplete:()=>h.destroy()});}this.time.delayedCall(1700,()=>this.showEnvelope());}});});}
  update(time,delta){if(this.rescuePlayable&&this.player){this.player.update(time,delta);const near=this.chains?.some(c=>!c.broken&&Math.abs(this.player.x-c.x)<105);this.chainPrompt?.setVisible(!!near);}}
  ...
  showLetter(){
    ...
    const content=this.add.container(0,0).setDepth(116);
    ...
    const handle=this.add.rectangle(1004,160,14,70,0xb94e78,.9).setInteractive({useHandCursor:true,draggable:true});this.input.setDraggable(handle);const touchZone=this.add.zone(viewport.x+viewport.w/2,viewport.y+viewport.h/2,viewport.w,viewport.h).setInteractive().setDepth(115);layer.add(touchZone);
    ...
    next.once('pointerdown',finish);this.input.on('wheel',onWheel);this.input.on('pointermove',onTouchMove);this.input.on('pointerup',onTouchUp);touchZone.on('pointerdown',onTouchDown);this.input.keyboard.on('keydown-UP',onUp);this.input.keyboard.on('keydown-DOWN',onDown);this.input.keyboard.on('keydown-PAGE_UP',onPageUp);this.input.keyboard.on('keydown-PAGE_DOWN',onPageDown);handle.on('dragstart',onDragStart);handle.on('drag',onDrag);this.events.once('shutdown',cleanup);render();
  }
  showCompleted(){
    const shade=this.add.rectangle(640,360,1280,720,0x13091d,.38).setDepth(120),anniversary=this.add.text(640,145,'FELIZ ANIVERSARIO MI AMOR ❤️',{fontFamily:'monospace',fontSize:'38px',color:'#fff0cc',stroke:'#71254f',strokeThickness:7}).setOrigin(.5).setDepth(123).setAlpha(0),continuation=this.add.text(640,225,'“Nuestra historia continúa…”',{fontFamily:'monospace',fontSize:'31px',color:'#ffe5af',stroke:'#71254f',strokeThickness:6}).setOrigin(.5).setDepth(122).setAlpha(0);for(let i=0;i<55;i++){const p=this.add.text(Math.random()*1280,-30-Math.random()*500,['♥','🌹','TE AMO','✦'][i%4],{fontFamily:'monospace',fontSize:`${12+Math.random()*18}px`,color:i%2?'#ff82b4':'#ffe39a'}).setDepth(121);this.tweens.add({targets:p,y:760,x:p.x+(Math.random()-.5)*170,angle:(Math.random()-.5)*120,duration:2800+Math.random()*2500,repeat:1,delay:Math.random()*900,onComplete:()=>p.destroy()});}this.tweens.add({targets:anniversary,alpha:1,scale:{from:.7,to:1},duration:850,ease:'Back.easeOut'});this.uiManager.showDialogueBubble('MATEO','Feliz aniversario, amor.',{x:820,y:430,tail:'right',duration:2200,width:500});this.time.delayedCall(2200,()=>this.uiManager.showDialogueBubble('PAOLA','❤️',{x:440,y:430,duration:1700,width:300}));this.time.delayedCall(3800,()=>this.tweens.add({targets:continuation,alpha:1,duration:800}));this.cameras.main.zoomTo(.9,6200,'Sine.easeInOut');this.time.delayedCall(6200,()=>{const fragments=(gameState.memories||[]).filter(x=>/^fragment\d+$/.test(x)).length;gameState.endingUnlocked=true;gameState.secretUnlocked=fragments===12;gameState.currentScene=fragments===12?'SecretScene':'MenuScene';gameState.achievements=[...new Set([...(gameState.achievements||[]),'FELIZ ANIVERSARIO ❤️'])];localStorage.setItem('rescate-de-amor-save',JSON.stringify(gameState));this.cameras.main.once('camerafadeoutcomplete',()=>this.scene.start(gameState.currentScene));this.cameras.main.fadeOut(700,8,4,18);});
  }
}
