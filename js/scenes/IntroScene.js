...
export default class IntroScene extends Phaser.Scene {
  ...
  flyToCastle(){
    ...
    this.locked=true;this.prompt.setText('LA JAULA VUELA HACIA EL CASTILLO…');this.pecho.setVisible(true).setAlpha(1).setFrame(1);this.aura.setVisible(true).setAlpha(1);this.paola.setFrame(2);this.bubble('PAOLA','¡MATEO!',510,420,'right',1200);this.time.delayedCall(500,()=>this.bubble('MATEO','¡PAOLA!',790,345,'left',1200));this.time.delayedCall(1450,()=>this.bubble('MATEO','Paola va a venir.',820,245,'left',1050));this.time.delayedCall(2350,()=>this.bubble('PECHO PALOMA','Eso espero.',1010,170,'right',900));this.tweens.add({targets:this.paola,x:900,duration:2700,ease:'Sine.easeInOut',onComplete:()=>this.paola.setFrame(5)});this.tweens.add({targets:this.cageRig,x:1510,y:120,duration:3900,ease:'Sine.easeIn'});this.tweens.add({targets:[this.pecho,this.aura],x:1600,y:95,duration:3850,ease:'Sine.easeIn'});for(let i=0;i<22;i++)this.time.delayedCall(i*150,()=>this.magicSpark(this.cageRig.x-20,this.cageRig.y+90));this.birds.forEach((b,i)=>this.tweens.add({targets:b,x:1450+(i%5)*35,y:70+(i%4)*35,duration:3300+i*35,ease:'Sine.easeIn'}));this.cameras.main.pan(760,330,2500,'Sine.easeInOut');this.cameras.main.zoomTo(1.06,2500,'Sine.easeInOut');this.time.delayedCall(4050,()=>{this.paola.setPosition(760,538).setFrame(5).setVisible(true).setAlpha(1);this.cameras.main.pan(640,360,650,'Sine.easeInOut');this.cameras.main.zoomTo(1,650,'Sine.easeInOut');this.step=16;this.showGardenResolve();});
  }
