...
export default class IntroScene extends Phaser.Scene {
  ...
  showGardenResolve(){
    this.locked=true;this.cameras.main.fadeOut(360,8,4,18);this.time.delayedCall(390,()=>{this.cageRig?.setVisible(false);this.villain.setVisible(false);this.aura.setVisible(false);this.birds.forEach(b=>b.setVisible(false));this.paola.setVisible(true).setPosition(640,535).setAngle(0).setFrame(9);const h=this.add.text(640,440,'♥',{fontSize:'36px',color:'#ff6fae'}).setOrigin(.5).setDepth(50);this.tweens.add({targets:h,y:405,scale:1.3,duration:700,yoyo:true,repeat:1});this.cameras.main.fadeIn(500,8,4,18);this.locked=false;this.say('PAOLA: “Voy a encontrarte.”\nPAOLA: “Y voy a traerte de vuelta.”');});
  }
  ...
  showTitle(){if(this.locked)return;this.locked=true;this.cinematicUi.activeDialogue?.dismiss?.(true);[this.paola,this.villain,this.cageRig].filter(Boolean).forEach(o=>o.setVisible(false));const title=this.add.text(640,290,'RESCATE DE AMOR ❤️\n\nPor ti, hasta el final.',{fontFamily:'monospace',fontSize:'35px',color:'#fff0cd',align:'center',lineSpacing:12,stroke:'#5b1e49',strokeThickness:7}).setOrigin(.5).setDepth(110).setAlpha(0),date=this.add.text(640,410,'19 • 09 • 2025',{fontFamily:'monospace',fontSize:'18px',color:'#ffe5b4',letterSpacing:3}).setOrigin(.5).setDepth(110).setAlpha(0),start=this.add.text(640,480,'[ COMENZAR RESCATE ]',{fontFamily:'monospace',fontSize:'20px',color:'#fff',backgroundColor:'#722752',padding:{x:18,y:10}}).setOrigin(.5).setDepth(111).setAlpha(0).setInteractive({useHandCursor:true}).once('pointerdown',()=>this.finish());this.tweens.add({targets:[title,date,start],alpha:1,duration:800});this.input.keyboard.once('keydown-ENTER',()=>this.finish());this.input.keyboard.once('keydown-SPACE',()=>this.finish());}
