...
export default class BaseLevelScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.makeTextures(); this.makeWorld(); this.makeGroundVisuals(); this.makeDecor(); this.makePlatforms(); this.validateRoute();
  ...
  spawnEnemyOnGround(enemy,x,intendedY){
    if(!enemy.humanoid)return;const feetTarget=intendedY+36,segments=this.dataDef.groundSegments||[{start:0,end:this.dataDef.width,top:648}],candidates=segments.filter(s=>x>=s.start&&x<=s.end).map(s=>({y:s.top??648,min:s.start+24,max:s.end-24}));this.dataDef.platforms.forEach(p=>{const width=p.renderWidth||Math.round(p.w*1.28);if(x>=p.x-width/2&&x<=p.x+width/2)candidates.push({y:p.y-13,min:p.x-width/2+18,max:p.x+width/2-18});});if(!candidates.length){console.error(`[QA ${this.scene.key}] Enemigo terrestre sin superficie`,enemy.type,x);enemy.disableBody(true,true);return;}candidates.sort((a,b)=>Math.abs(a.y-feetTarget)-Math.abs(b.y-feetTarget));const surface=candidates[0];enemy.setPosition(x,surface.y-40);enemy.body.updateFromGameObject();enemy.y+=surface.y-enemy.body.bottom;enemy.body.updateFromGameObject();enemy.safeSpawn={x,y:enemy.y};enemy.spawnX=x;enemy.minX=Math.max(enemy.minX,surface.min);enemy.maxX=Math.min(enemy.maxX,surface.max);if(enemy.minX>enemy.maxX){enemy.minX=surface.min;enemy.maxX=surface.max;}enemy.patrolMinX=enemy.minX;enemy.patrolMaxX=enemy.maxX;enemy.body.setAllowGravity(true);
  }

  spawnEnemyOnPlatform(enemy,platform){
    if(!enemy?.humanoid||!platform)return;const width=platform.renderWidth||Math.round(platform.w*1.28),surfaceTop=platform.y-13,x=Phaser.Math.Clamp(enemy.x,platform.x-width/2+22,platform.x+width/2-22);enemy.setPosition(x,surfaceTop-45);enemy.body.updateFromGameObject();enemy.y+=surfaceTop-enemy.body.bottom;enemy.body.updateFromGameObject();enemy.safeSpawn={x:enemy.x,y:enemy.y};enemy.spawnX=enemy.x;enemy.minX=platform.x-width/2+22;enemy.maxX=platform.x+width/2-22;enemy.patrolMinX=enemy.minX;enemy.patrolMaxX=enemy.maxX;enemy.body.setAllowGravity(true);
  }
  ...
  makeWorld() {
    this.physics.world.setBounds(0,0,this.dataDef.width,this.dataDef.groundSegments?900:720); this.cameras.main.setBackgroundColor(this.dataDef.sky);
    ...
    this.solids=this.physics.add.staticGroup();(this.dataDef.groundSegments||[{start:0,end:this.dataDef.width,top:648}]).forEach(s=>{const top=s.top??648,ground=this.add.rectangle((s.start+s.end)/2,top+36,s.end-s.start,72,0x000000,0).setVisible(false);this.physics.add.existing(ground,true);this.solids.add(ground);});
  }

  makeGroundVisuals(){const segments=this.dataDef.groundSegments;if(!segments)return;segments.forEach(s=>{for(let x=s.start;x<s.end;x+=520){const width=Math.min(540,s.end-x);this.add.image(x+width/2,(s.top??648)+43,'environment-atlas',0).setDisplaySize(width+8,122).setDepth(9);}});(this.dataDef.visualSections||[]).forEach(section=>{this.add.image((section.start+section.end)/2,360,section.background||'bg-forest').setDisplaySize(section.end-section.start,720).setTint(section.tint??0xffffff).setDepth(-15);if(section.arches)for(let x=section.start+170;x<section.end;x+=330)this.add.image(x,455,'environment-atlas',13).setDisplaySize(300,410).setDepth(5);});}
  ...
  makeHazards(){const layouts={forest:[880,1430,2240,2760,3220],garden:[830,1610,2400,3190,3840],castle:[820,1590,2370,3150,3900],tower:[1050,1880,2760,3660,4070],palace:[1120,2010,2920,3780]},styles={forest:[8,8,11],garden:[8,9,8],castle:[9,10,11],tower:[10,9,11],palace:[11,10,9]},defs=this.dataDef.hazards||(layouts[this.dataDef.theme]||[]).map((x,i)=>({x,y:625,frame:styles[this.dataDef.theme][i%3]}));this.hazards=[];defs.forEach((def,i)=>{const frame=def.frame,key=['','','','','','','','','hazard-thorns','hazard-flame','hazard-blade','hazard-magic'][frame],h=this.add.image(def.x,def.y,'environment-atlas',frame).setName(key).setDepth(12).setDisplaySize(frame===10?72:frame===8?86:78,frame===10?72:62);h.baseScaleX=h.scaleX;h.baseScaleY=h.scaleY;this.physics.add.existing(h,true);h.body.setSize(Math.min(58,h.displayWidth*.72),Math.min(30,h.displayHeight*.42));h.phase=i*430;h.warning=this.add.ellipse(def.x,def.y+21,74,24,this.dataDef.accent,.08).setStrokeStyle(2,this.dataDef.accent,.65).setDepth(11);this.hazards.push(h);this.physics.add.overlap(this.player,h,()=>{if(h.dangerous)this.player.takeDamage(1,h);});});}
  ...
  makeLevelPuzzle(){this.puzzleSolved=false;this.puzzlePrompt=this.add.text(640,590,'',{fontFamily:'monospace',fontSize:'16px',color:'#fff2d6',backgroundColor:'#29162d',padding:{x:14,y:8},align:'center'}).setOrigin(.5).setScrollFactor(0).setDepth(1100).setVisible(false);
    if(this.scene.key==='Level1Scene'){this.puzzleX=this.dataDef.puzzleX??1480;this.puzzleDoor=this.makePuzzleDoor(this.puzzleX,'PUERTA DEL\nRECUERDO');this.input.keyboard.on('keydown',event=>{if(!this.puzzleActive||this.puzzleSolved)return;const answer=event.key?.toUpperCase();if(answer==='D'){this.puzzleActive=false;this.puzzlePrompt.setVisible(false);this.openPuzzleDoor('Todo ❤️. Siempre fue todo.');}else if(['A','B','C'].includes(answer))this.uiManager.showDialogueBubble('PUERTA DEL RECUERDO','También cuenta... pero ambos sabemos la respuesta 😏',{x:640,y:390,duration:1700,width:650});});}
  ...
  makeExit(){
    ...
    const arch=this.dataDef.theme==='forest'?this.add.image(0,-4,'environment-atlas',12).setDisplaySize(190,220):this.add.graphics();
    if(this.dataDef.theme==='forest'){
      arch.setTint(0xffd7ee);
  ...
  update(time,delta){
    ...
    this.sectionMarkers?.forEach(section=>{if(!section.shown&&this.player.x>=section.x){section.shown=true;this.cameras.main.fadeOut(120,7,4,18);this.time.delayedCall(135,()=>{this.cameras.main.fadeIn(260,7,4,18);this.uiManager.showMessage(section.label,'#ffe6af',1500);});}});
