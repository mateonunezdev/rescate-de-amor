import Player from '../entities/Player.js?v=20260902-level1-gold-01';
import Enemy from '../entities/Enemy.js?v=20260902-level1-gold-01';
...
import WorldBuilder from '../world/WorldBuilder.js?v=20260901-grounded-final-04';
...
export default class BaseLevelScene extends Phaser.Scene {
  ...
  create() {
    ...
    if(this.scene.key==='Level1Scene')this.time.delayedCall(0,()=>this.puzzlePrompt?.setY(430));
  ...
  makeLevelPuzzle(){this.puzzleSolved=false;this.puzzlePrompt=this.scene.key==='Level1Scene'?this.makeMemoryPuzzlePanel():this.add.text(640,590,'',{fontFamily:'monospace',fontSize:'16px',color:'#fff2d6',backgroundColor:'#29162d',padding:{x:14,y:8},align:'center'}).setOrigin(.5).setScrollFactor(0).setDepth(1100).setVisible(false);
    ...
    else{const hearts=this.dataDef.puzzleType==='hearts';this.puzzleX=this.dataDef.puzzleX;this.puzzleDoor=this.makePuzzleDoor(this.puzzleX,hearts?'PUERTA DEL\nFARO':'PASADIZO\nDE LA BIBLIOTECA');const defs=hearts?[{x:980,y:1130,icon:'♥'},{x:2070,y:730,icon:'♥'},{x:3000,y:430,icon:'♥'}]:[{x:900,y:575,icon:'I'},{x:2250,y:575,icon:'II'},{x:2500,y:575,icon:'III'}];this.puzzleSymbols=defs.map(def=>{const node=this.add.text(def.x,def.y,def.icon,{fontFamily:'monospace',fontSize:'28px',color:hearts?'#ff72b1':'#f3c66f',backgroundColor:'#28182f',padding:{x:10,y:7}}).setOrigin(.5).setDepth(22);this.tweens.add({targets:node,scale:1.12,alpha:.72,duration:700,yoyo:true,repeat:-1});return{...def,node,active:false};});this.input.keyboard.on('keydown-E',()=>this.activateWorldSymbol());}
  }

  makeMemoryPuzzlePanel(){const layer=this.add.container(640,430).setScrollFactor(0).setDepth(1100).setVisible(false),g=this.add.graphics();g.fillStyle(0x120a18,.92).fillRect(-270,-112,540,224).fillStyle(0xe2b76e,1).fillRect(-262,-104,524,4).fillRect(-262,100,524,4).fillStyle(0x30182d,1).fillRect(-262,-96,524,188);const title=this.add.text(0,-78,'🌹 PREGUNTA DEL RECUERDO 🌹',{fontFamily:'monospace',fontSize:'16px',color:'#ffe4a7'}).setOrigin(.5),question=this.add.text(0,-43,'¿Qué es lo que más te gusta de Mateo?',{fontFamily:'monospace',fontSize:'16px',color:'#fff1df'}).setOrigin(.5);const defs=[['A','Su risa'],['B','Cómo me cuida'],['C','Su forma de ser'],['D','Todo ❤️']];const options=defs.map(([key,label],i)=>{const col=i%2,row=Math.floor(i/2);return this.add.text(col?-2:-246,-2+row*50,`${key}) ${label}`,{fontFamily:'monospace',fontSize:'15px',color:key==='D'?'#ffd1e3':'#efe1d4',backgroundColor:'#211225',padding:{x:8,y:7}}).setInteractive({useHandCursor:true}).on('pointerover',function(){this.setText(`♥ ${key}) ${label}`).setColor('#fff0a5');}).on('pointerout',function(){this.setText(`${key}) ${label}`).setColor(key==='D'?'#ffd1e3':'#efe1d4');}).on('pointerdown',()=>this.answerMemoryPuzzle(key));});layer.add([g,title,question,...options]);layer.setText=()=>layer;return layer;}

  answerMemoryPuzzle(answer){if(!this.puzzleActive||this.puzzleSolved||!['A','B','C','D'].includes(answer))return;if(answer==='D'){this.puzzleActive=false;this.puzzlePrompt.setVisible(false);this.openPuzzleDoor('Todo ❤️. Siempre fue todo.');}else this.uiManager.showDialogueBubble('PUERTA DEL RECUERDO','También cuenta... pero ambos sabemos la respuesta 😏',{x:640,y:390,duration:1700,width:470,variant:'thought'});}
  ...
  makeExit(){
    ...
    const arch=this.dataDef.theme==='forest'?this.add.image(0,72,'forest-remaster-atlas','gardenPortal').setDisplaySize(390,300).setOrigin(.5,1):this.add.graphics();
