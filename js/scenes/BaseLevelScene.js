...
export default class BaseLevelScene extends Phaser.Scene {
  ...
  makeHazards(){const layouts={forest:[620,880,1160,1430,1690,1970,2240,2500,2760,3220],garden:[580,830,1090,1350,1610,1880,2140,2400,2670,2930,3190,3450,3840],castle:[570,820,1080,1330,1590,1850,2110,2370,2630,2890,3150,3410,3670,3900,4310],tower:[650,1050,1460,1880,2320,2760,3210,3660,4070],palace:[680,1120,1560,2010,2460,2920,3380,3780]},styles={forest:[8,8,11],garden:[8,9,8],castle:[9,10,11],tower:[10,9,11],palace:[11,10,9]};this.hazards=[];(layouts[this.dataDef.theme]||[]).forEach((x,i)=>{const frame=styles[this.dataDef.theme][i%3],key=['','','','','','','','','hazard-thorns','hazard-flame','hazard-blade','hazard-magic'][frame],h=this.add.image(x,625,'environment-atlas',frame).setName(key).setDepth(12).setDisplaySize(frame===10?72:frame===8?86:78,frame===10?72:62);h.baseScaleX=h.scaleX;h.baseScaleY=h.scaleY;this.physics.add.existing(h,true);h.body.setSize(Math.min(58,h.displayWidth*.72),Math.min(30,h.displayHeight*.42));h.phase=i*430;h.warning=this.add.ellipse(x,646,74,24,this.dataDef.accent,.08).setStrokeStyle(2,this.dataDef.accent,.65).setDepth(11);this.hazards.push(h);this.physics.add.overlap(this.player,h,()=>{if(h.dangerous)this.player.takeDamage(1,h);});});}
