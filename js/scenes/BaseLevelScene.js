...
export default class BaseLevelScene extends Phaser.Scene {
  ...
  makePlatforms(){this.dataDef.platforms.forEach((p,i)=>{const pw=Math.round(p.w*(i%5===0?1.38:i%3===0?1.3:1.24));p.renderWidth=pw;
    const r=this.add.rectangle(p.x,p.y,pw,26,0x000000,0);this.physics.add.existing(r,true);this.solids.add(r);
    const pieces=Math.ceil(pw/46),pieceW=pw/pieces;for(let n=0;n<pieces;n++){const px=p.x-pw/2+pieceW*(n+.5),w=pieceW-2;if(this.dataDef.theme==='forest'){const rock=this.add.ellipse(px,p.y+(n%2?1:-1),w+7,27,n%3===0?0x4d4037:0x46504a).setStrokeStyle(2,0x282f2b,.9).setDepth(7);if(n%2===0)this.add.line(0,0,px-w*.2,p.y-4,px+w*.16,p.y+8,0x756650,.8).setOrigin(0,0).setDepth(8);}else if(this.dataDef.theme==='garden'){this.add.polygon(px,p.y,[[-w/2,-12],[w/2-3,-12],[w/2,8],[w/3,12],[-w/2+2,10]],n%2?0xa89c99:0x91888d).setStrokeStyle(2,0xd2c1ad,.8).setDepth(7);if(n%3===0)this.add.line(0,0,px-8,p.y-8,px+5,p.y+7,0x675d68,.6).setOrigin(0,0).setDepth(8);}else{this.add.polygon(px,p.y,[[-w/2,-12],[w/2,-12],[w/2-2,11],[-w/2+3,11]],n%2?0x393441:0x494052).setStrokeStyle(2,0x75627d,.7).setDepth(7);this.add.rectangle(px,p.y-9,w-5,2,0x8a718d,.55).setDepth(8);}}
    for(let k=0;k<Math.max(3,pw/28);k++){const x=p.x-pw/2+12+k*28;if(this.dataDef.theme==='forest'){this.add.ellipse(x,p.y-15,18,7,k%2?0x617758:0x728864).setDepth(8);if(k%3===0)this.add.ellipse(x+7,p.y+5,22,5,0x3d6545,.8).setDepth(9);}else if(this.dataDef.theme==='garden'){this.add.arc(x,p.y-21,8,180,360,false,0x3f7049,.9).setDepth(8);if(k%2===0)this.add.circle(x+5,p.y-24,4,k%4?0xe05288:0xff9ebc).setDepth(9);}else{this.add.rectangle(x,p.y,2,19,0x382d46,.8).setDepth(8);if(k%3===0)this.add.text(x,p.y-2,'ᚱ',{fontFamily:'monospace',fontSize:'11px',color:'#c078b8'}).setOrigin(.5).setDepth(9);}}
