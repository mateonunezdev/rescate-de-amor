...
export default class BaseLevelScene extends Phaser.Scene {
  ...
  create() {
    this.exitCreated=false;this.finishing=false;this.paused=false;this.gameOver=false;
  ...
  collect(item) {
    if(item.collected)return; const type=item.type; item.collect();
    if(item.isMemory){ SaveManager.addUnique('memories',type); if(type==='heart'){ gameState.maxHealth=Math.max(6,gameState.maxHealth); this.player.maxHealth=gameState.maxHealth; this.player.heal(2); } if(type==='rose')this.player.heal(2); if(type==='diamond'){this.player.jumpVelocity=-570; gameState.power='super salto';}if(type==='card1'){gameState.maxHealth=Math.max(6,gameState.maxHealth);this.player.maxHealth=gameState.maxHealth;this.player.heal(3);}if(type==='card2')gameState.attackBoost=1.25;if(type==='card3'){gameState.chargedUnlocked=true;this.player.canChargeLove=true;}
      SaveManager.addUnique('achievements',type.startsWith('card')?`Carta ${type.slice(-1)} conseguida`:type==='heart'?'Primer recuerdo':type==='diamond'?'Más alto que mis sueños':'Recuerdo encontrado');type.startsWith('card')?this.showCard(type):this.showMemory(type);
    } else this.addRose(1); this.refreshHud(); SaveManager.save(gameState);
  }

  showCard(type){const [name,msg]=MEMORY[type];const layer=this.add.container(0,0).setScrollFactor(0).setDepth(330);this.cardOverlay=layer;const shade=this.add.rectangle(640,360,1280,720,0x17091f,.52);const paper=this.add.rectangle(640,345,620,270,0xffefd5,.98).setStrokeStyle(8,0xd45586);const title=this.add.text(640,270,`💌  ${name} CONSEGUIDA  ❤️`,{fontFamily:'monospace',fontSize:'27px',color:'#72264f',fontStyle:'bold'}).setOrigin(.5);const line=this.add.text(640,365,`“${msg}”\n\nSALIDA DESBLOQUEADA`,{fontFamily:'monospace',fontSize:'22px',color:'#51203e',align:'center',lineSpacing:10}).setOrigin(.5);layer.add([shade,paper,title,line]);layer.setScale(.7).setAlpha(0);this.tweens.add({targets:layer,scale:1,alpha:1,duration:330,ease:'Back.easeOut'});this.particleManager.burst(640,345,0xff6eac,28,260);this.audioManager.playSfx('checkpoint');this.time.delayedCall(1650,()=>{if(!layer.active)return;this.tweens.add({targets:layer,alpha:0,y:-35,duration:280,onComplete:()=>{if(this.cardOverlay===layer)this.cardOverlay=null;layer.destroy();}});});SaveManager.save(gameState);}
  ...
  refreshHud(){ this.uiManager?.updateHud({health:this.player?.health||gameState.health,roses:gameState.roses,letters:gameState.memories.length,power:gameState.power}); }
