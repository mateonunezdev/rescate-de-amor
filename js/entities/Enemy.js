export default class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  rangedAttack(player){this.setTint(this.type==='mage'?0xc774ff:0xffd277);const mark=this.scene.add.line(0,0,this.x,this.y-15,player.x,player.y,0xff78be,.45).setOrigin(0,0).setDepth(18);this.scene.time.delayedCall(420,()=>{if(!this.active){mark.destroy();return;}mark.destroy();this.clearTint();const orb=this.scene.add.circle(this.x,this.y-12,this.type==='mage'?9:6,this.type==='mage'?0xa64fd0:0xffd6e6,1).setDepth(19);this.scene.physics.add.existing(orb);orb.body.setAllowGravity(false);const angle=Phaser.Math.Angle.Between(this.x,this.y,player.x,player.y);orb.body.setVelocity(Math.cos(angle)*260,Math.sin(angle)*260);const hit=this.scene.physics.add.overlap(player,orb,()=>{player.takeDamage(1);hit.destroy();orb.destroy();});this.scene.time.delayedCall(2200,()=>{hit.active&&hit.destroy();orb.active&&orb.destroy();});});}
